...
$environments = @{
    ...
    "Candidates" = @{
        ...
        LocalSrc  = "$baseDir\student-onboarding"
...
function Deploy-Api($env) {
    ...
    $apiFiles = @("server.js","websocket.js","package.json","package-lock.json","public","routes","middleware","database",".env","db","utils","services","cron")
...
foreach ($envKey in $selected) {
    ...
    $ok = if ($env.Type -eq "Api") { Deploy-Api $env } elseif ($env.Type -eq "Django") { Deploy-Django $env } else { Deploy-Frontend $env }
