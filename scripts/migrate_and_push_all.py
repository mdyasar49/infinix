    ...
    try:
        ...
        if res.returncode == 0:
            ...
            if "PUSH PROTECTION" in err or "violations found" in err:
                print(f"  [⚠️] {repo_name}: Blocked by GitHub Secret Scanning (contains API key/credentials file in history)")
            else:
                print(f"  [!] {repo_name}: Push error -> {err.strip()[:150]}")
            return False
    except Exception as e:
...
def main():
    print("=" * 80)
    print(" 🚀 COMPLETE CREATION & PUSH AUDIT FOR ALL 13 REPOSITORIES")
    print("=" * 80)
    ...
    for repo_name, success in status_summary:
        icon = "✅ Pushed & Active" if success else "⚠️ Created on GitHub (Needs Push Protection Bypass / Clean Commit)"
